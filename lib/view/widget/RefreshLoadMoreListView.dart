...
class RefreshLoadmoreOption {
  ...
  RefreshLoadmoreOption({Key key, this.canLoadMore = true, this.canRefresh = true, this.loadMoreItem});
}

class _RefreshLoadmoreListViewState extends State<RefreshLoadmoreListView> with AutomaticKeepAliveClientMixin {
  final RLDataFactory _dataFactory;
  final RefreshLoadmoreOption _option;
          ...
          builder: (context, snapshot) {
            ...
            return RefreshIndicator(
              child: ListView.separated(
                ...
                itemCount: _pageHelper.datas.length + ((_option.canLoadMore && !_pageHelper.isLastPage) ? 1 : 0),
                itemBuilder: (context, index) {
                  if (_option.canLoadMore) {
                    if (!_pageHelper.isLastPage && index == _pageHelper.datas.length) {
                      return _option.loadMoreItem ??
                          Center(
                            child: Layout(
                                child: CircularProgressIndicator(
                              strokeWidth: 3.0,
                            )).backgroundColor(Colors.white).size(width: 30, height: 30).margin(top: 8, bottom: 8),
                          );
                    } else {
                      return _dataFactory.createItemWidget(context, index, _pageHelper.datas[index]);
                    }
                  }
                  return _dataFactory.createItemWidget(context, index, _pageHelper.datas[index]);
                },
                separatorBuilder: (BuildContext context, int index) {
                  return Layout().size(height: 24).backgroundColor(Colors.white12);
                },
              ),
              onRefresh: () => refresh(),
            );
